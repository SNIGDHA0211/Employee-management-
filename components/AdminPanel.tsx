...
import { UserPlus, Trash2, Shield, Calendar, Mail, User as UserIcon, Upload, Hash, Camera, Lock, Key, Building2, X, Briefcase, Users as UsersIcon, Pencil, Check, XCircle } from 'lucide-react';
import api, { 
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-50">
                {displayUsers.length === 0 ? (
                  ...
                  <tr 
                    key={user.id} 
                    className={`group hover:bg-gray-50 ${editingUserId === user.id ? 'bg-blue-50' : 'cursor-pointer'}`}
                    onClick={() => {
                      // Don't open sidebar if we're editing this row
                      if (editingUserId === user.id) return;
                      // Always try to get user from apiUsers first (which has rawRole)
                      const userWithRawRole = apiUsers.find(u => u.id === user.id) || user;
                      console.log(`🔍 [ADMIN PANEL] Opening profile for ${user.name}:`, {
                        rawRole: (userWithRawRole as any).rawRole,
                        role: userWithRawRole.role,
                        hasRawRole: !!(userWithRawRole as any).rawRole,
                        source: apiUsers.find(u => u.id === user.id) ? 'apiUsers' : 'users prop'
                      });
                      setSelectedUser(userWithRawRole);
                    }}
                  >
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-lg font-bold text-gray-800 mt-1">{selectedUser.id}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditMode ? (
                      ...
                      <p className="text-lg font-semibold text-gray-800 mt-1">{selectedUser.name}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditMode ? (
                      ...
                      <p className="text-lg font-semibold text-gray-800 mt-1">{selectedUser.designation || 'N/A'}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditMode ? (
                      ...
                      <p className="text-lg font-semibold text-gray-800 mt-1">{selectedUser.email || 'N/A'}</p>
                    )}
                  </div>
                </div>
                ...
                {isEditMode && (
                  <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">New Password (Optional)</p>
                      <input
                        type="password"
