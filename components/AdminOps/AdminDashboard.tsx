...
const AdminDashboard: React.FC<DashboardProps> = ({ assets, bills, expenses, vendors }) => {
  ...
  const billsData = useMemo(() => {
    ...
    bills.forEach((b) => {
      categories[b.category] = (categories[b.category] || 0) + (Number(b.amount) || 0);
    });
  ...
  const COLORS = ['#f97316', '#10b981', '#f43f5e', '#f59e0b'];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold mb-6 text-gray-800">Expense Trend (Monthly Comparison)</h3>
          <div className="h-80">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex space-x-2">
              <button className="px-3 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-full">Monthly</button>
              <button className="px-3 py-1 bg-gray-50 text-gray-400 text-xs font-bold rounded-full hover:bg-gray-100">Yearly</button>
            </div>
          </div>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={billsData}>
                ...
                <Bar dataKey="amount" fill="#4f46e5" radius={[4, 4, 0, 0]} name="Current Period" />
                <Bar dataKey="lastMonth" fill="#e2e8f0" radius={[4, 4, 0, 0]} name="Last Period" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold mb-6 text-gray-800">Asset Distribution</h3>
          <div className="h-80 flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={donutData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold mb-4 text-gray-800">Vendor Overview</h3>
          <div className="space-y-4">
            {vendors.slice(0, 4).map((v) => (
              <div key={v.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                ...
                <span className="text-xs font-bold text-gray-400"># {v.gstNumber}</span>
              </div>
            ))}
            {vendors.length === 0 && <p className="text-center text-gray-400 py-4">No vendors added yet.</p>}
          </div>
        </div>
      </div>
    </div>
