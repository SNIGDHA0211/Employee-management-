...
const BillsManager: React.FC<BillsManagerProps> = ({ bills, setBills }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const data = await getBillCategories();

        console.log("Bill categories from API:", data); // 👈 ADD THIS
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Track utility and maintenance expenses
          </p>
        </div>
        <button
        ...
        >
          {showForm ? (
            ...
            <>
              <Plus size={20} />{" "}
              <span className="font-semibold">Create Bill Record</span>
            </>
          )}
        </button>
      </div>
          ...
          >
            <div>
              ...
              >
                ...
                {loading && <option className="text-black">Loading...</option>}

                {!loading &&
                  categories.map((cat: any) => {
                    const label = cat.category || cat.name || cat.title;
                    return (
                      <option
                        key={cat.id ?? label}
                        value={label}
